...
function wireAddAllSmart(){
  ...
  btn.addEventListener("click", ()=>{
    ...
    const a=document.createElement("a"); a.href=url; a.download="Gator_Hoops_Schedule.ics"; a.click();
...
function renderSchedule(list){
  ...
  for(const g of list){
    ...
    actions.appendChild(el(`<a class="btn" target="_blank" rel="noopener" href="${tix.seatgeek}">SeatGeek</a>`));
    actions.appendChild(el(`<a class="btn" target="_blank" rel="noopener" href="${tix.ticketmaster}">TM</a>`));
    actions.appendChild(el(`<a class="btn" target="_blank" rel="noopener" href="${tix.vivid}">Vivid</a>`));
    actions.appendChild(el(`<a class="btn" target="_blank" rel="noopener" href="${tix.google}">Google</a>`));
...
function renderRoster(players){
  ...
  for(const p of players){
    ...
    const card=el(`
      <div class="card">
        <div class="row">
          ${p.headshot?`<img class="head" alt="" src="${p.headshot}" onerror="this.onerror=null;this.src='https://upload.wikimedia.org/wikipedia/commons/7/7d/Florida_Gators_gator_logo.svg'">`:`<div class="head"></div>`}
          <div style="flex:1">
            <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
              <div style="font-weight:800">${p.fullName}</div>
              <span class="meta">${p.number?("#"+p.number+" "):""}${p.position??""} ${p.classYear?("· "+p.classYear):""} ${p.height?("· "+p.height):""}${p.weight?(" / "+p.weight):""}</span>
            </div>
            <div class="statgrid">
...
function renderPropsCards(){
  ...
  selected.forEach(p=>{
    const avg=p.stats?.ppg||0, line=makeLine(avg), sd=defaultSd(avg);
    const pOver=Math.round(overProb(line, avg, sd)*100);
    const div=el(`
      <div class="prop-card">
        <div>
          <div class="prop-title">${p.fullName}</div>
          <div class="prop-meta">Season: ${(avg||0).toFixed(1)} PPG</div>
        </div>
        <div style="display:flex;gap:12px;align-items:center">
          <div class="toggle" data-player="${p.id}">
    ...
    div.querySelector(".toggle").addEventListener("click", async (e)=>{
      ...
      // Save pick to Supabase for leaderboard
      try{
        const uid=getUserId();
        const name=localStorage.getItem("gh_name") || `Fan-${uid.slice(0,6)}`;
        await upsertUserName(name);
        await sb.from("prop_picks").insert({
          user_id: uid, event_id: selectedEventId, player_id: p.id,
          line: Number(line), choice: btn.dataset.type
        });
      }catch(err){ console.warn("save pick failed", err); }
    });
    propsWrap.appendChild(div);
  });
}

/* ===== Odds via Netlify function ===== */
async function loadGameOdds(){
  if(!gameOddsDiv) return;
  gameOddsDiv.innerHTML = "<div class='note'>Loading odds…</div>";
  try{
    const res = await fetch(`${NETLIFY_ODDS_FN}?type=game&region=us`);
    ...
    for(const event of odds){
      ...
      const match = sched.find(g=>{
        const opp = normalizeName(g.opponent);
        const us = "florida gators";
        return g.isHome ? (normalizeName("florida gators")===us && opp===away)
                        : (normalizeName("florida gators")===us && opp===home);
      });
      if(!match) continue;
      ...
      const card=el(`
        <div class="card" style="cursor:pointer" title="Tap to view player props">
          <div class="row">
            <div style="flex:1">
      ...
      `);
      card.addEventListener("click", ()=> loadPlayerProps(event.id, match));
      rows.push(card);
    }

    gameOddsDiv.innerHTML = rows.length ? "" : "<div class='note'>No Gators games with odds right now.</div>";
...
async function loadPlayerProps(oddsEventId){
  ...
  try{
    const res = await fetch(`${NETLIFY_ODDS_FN}?type=player&eventId=${encodeURIComponent(oddsEventId)}&region=us`);
      ...
      (mkt.outcomes||[]).forEach(o=>{
        list.appendChild(el(`
          <div class="row">
            ...
            <div class="meta">${o.point!=null?`Line: ${o.point}`:""}</div>
            <div class="right">${o.price>0?"+":""}${o.price}</div>
          </div>`));
      });
      blocks.push(list);
    }
    ...
    playerPropsDiv.innerHTML = `<div class='note'>Failed to load player props (${String(e)}).</div>`;
  }
}
...
if(lbSave){ lbSave.addEventListener("click", async ()=>{ const name=(lbName.value||"").trim().slice(0,40); if(!name) return alert("Enter a display name"); await upsertUserName(name); renderLeaderboard(); alert("Saved! Your picks will count on the board."); }); }
...
/* ===== Resolve picks after a finished game (best effort) ===== */
async function resolvePicksIfPossible(games){
  const lastFinished = [...games].reverse().find(g=> g.myScore!=null && g.oppScore!=null);
  if(!lastFinished) return;
  const { data: openPicks } = await sb.from("prop_picks").select("*").eq("event_id", lastFinished.id).eq("resolved", false);
  if(!openPicks?.length) return;

  let summary=null; try{ summary = await getJSON(`${BASE}/${GENDER}/summary?event=${encodeURIComponent(lastFinished.id)}`);}catch{ return; }
  const box = summary?.boxscore?.teams?.find(t=> String(t.team?.id)===String(TEAM_ID)); if(!box) return;

  const ptsById = {}; (box.athletes||[]).forEach(a=>{ const pid=String(a.athlete?.id); const pts=Number(a.stats?.find(s=>s.name==="points")?.value || 0); ptsById[pid]=pts; });

  const updates = [];
  for(const pick of openPicks){
    const actual = ptsById[pick.player_id];
    if(actual==null) continue;
    const correct = pick.choice === "over" ? actual > pick.line : actual < pick.line;
    updates.push({ id: pick.id, resolved:true, correct });
  }
  if(updates.length) await sb.from("prop_picks").upsert(updates);
}
...
async function loadAll(){
  try{
    ...
    try{
      rosterJSON = await getJSON(`${BASE}/${GENDER}/teams/${TEAM_ID}`);
    ...
    loadGameOdds();
    await resolvePicksIfPossible(games);
    renderLeaderboard();
  }catch(err){
...
document.addEventListener("DOMContentLoaded", ()=>{
  ...
  wireAddAllSmart();
  loadAll();
  if(makePropsBtn) makePropsBtn.addEventListener("click", renderPropsCards);
});
